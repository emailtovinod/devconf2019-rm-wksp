...
. Now you may copy the URL and paste in your workstation browser address bar or in plain and simple, access the URL using a browser ( I may recommend using chrome/firefox)
...
[NOTE]
Even though the QoS remains the same - Burstable - as in the case of `request` only setting, here we set the `limit` for the run time resource consumption of the POD. 
...
[NOTE]
Now the QoS of the POD changed from `Burstable` to `Guaranteed`. Pods with `Guaranteed QoS Class` are considered top-priority and are guaranteed not to be killed until they exceed their limits.
